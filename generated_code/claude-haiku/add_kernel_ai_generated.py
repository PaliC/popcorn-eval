...
def add_kernel(x_ptr,  # Pointer to first input vector
    ...
    # Calculate the current program's starting index
    pid = tl.program_id(axis=0)  # Get program ID
    block_start = pid * BLOCK_SIZE  # Calculate start of current block
    
    # Create an offset range within the block
    offsets = block_start + tl.arange(0, BLOCK_SIZE)
    
    # Create mask to handle boundary conditions
    mask = offsets < n_elements
    
    # Load input vectors with boundary mask
