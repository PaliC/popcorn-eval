...
def _layer_norm_bwd_dx_fused(DX, DY, DW, DB, X, W, Mean, Rstd, Lock, 
    ...
    # Compute start offset for this thread
    offs_n = pid_m * BLOCK_SIZE_N + tl.arange(0, BLOCK_SIZE_N)
    offs_m = pid_g * GROUP_SIZE_M + tl.arange(0, GROUP_SIZE_M)
    # Create mask for valid memory
    mask = offs_n < N

    # Load data for this thread
    x = tl.load(X + offs_n, mask=mask)
    dy = tl.load(DY + offs_n, mask=mask)
    ...
    # Compute dx_hat
    dy_w = dy * w
    # First two terms of dx
    dx = dy_w * rstd
    
    # Compute reduction terms
    dy_sum = tl.sum(dy_w, axis=0)
    dy_xhat_sum = tl.sum(dy_w * xhat, axis=0)
    
    # Complete dx computation
    dx = dx - (dy_sum * rstd + xhat * dy_xhat_sum * rstd ** 2) / N
    
    # Store dx
    tl.store(DX + offs_n, dx, mask=mask)

    # Accumulate dw and db
    dw = dy * xhat
    db = dy
    
    # Use atomic adds for weight/bias gradients
    for i in range(BLOCK_SIZE_N):
        if mask[i]:
            tl.atomic_add(DW + offs_n[i], dw[i])
            tl.atomic_add(DB + offs_n[i], db[i])
