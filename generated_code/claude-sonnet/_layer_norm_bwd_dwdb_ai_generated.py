...
def _layer_norm_bwd_dwdb(DW,  # pointer to the partial sum of weights gradient
    ...
    """
    Accumulates partial gradients for weights and biases in layer normalization
    """
    # Calculate indices
    pid = tl.program_id(0)
    cols = tl.arange(0, BLOCK_SIZE_N)
    ...
    acc_dw = tl.zeros([BLOCK_SIZE_N], dtype=tl.float32)
    acc_db = tl.zeros([BLOCK_SIZE_N], dtype=tl.float32)

    # Accumulate over rows
    for i in range(0, M, BLOCK_SIZE_M):
        row_idx = i + tl.arange(0, BLOCK_SIZE_M)
        row_mask = row_idx < M
        
        # Load partial sums 
        dw = tl.load(DW + row_idx[:, None] * N + cols[None, :], 
                    mask=row_mask[:, None] & mask[None, :],
                    other=0.0)
        db = tl.load(DB + row_idx[:, None] * N + cols[None, :],
                    mask=row_mask[:, None] & mask[None, :],
                    other=0.0)

        # Accumulate
        acc_dw += tl.sum(dw, axis=0)
        acc_db += tl.sum(db, axis=0)

    # Store final gradients
    tl.store(FINAL_DW + cols, acc_dw, mask=mask)
    tl.store(FINAL_DB + cols, acc_db, mask=mask)
