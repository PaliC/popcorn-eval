...
def _layer_norm_bwd_dwdb(DW,  # pointer to the partial sum of weights gradient
    ...
    # Map program id to the row of the output it should compute
    pid = tl.program_id(0)
    cols = pid * BLOCK_SIZE_N + tl.arange(0, BLOCK_SIZE_N)
    mask = cols < N
    
    # Compute partial sums
    dw_sum = tl.zeros([BLOCK_SIZE_N], dtype=tl.float32)
    db_sum = tl.zeros([BLOCK_SIZE_N], dtype=tl.float32)
    
    # Iterate through rows to compute partial sums
    for i in range(0, M, BLOCK_SIZE_M):
        row_mask = tl.arange(0, BLOCK_SIZE_M) < M - i
        
        # Load partial gradients
        dw = tl.load(DW + i * N + cols, mask=mask & row_mask[:, None], other=0.0)
        db = tl.load(DB + i * N + cols, mask=mask & row_mask[:, None], other=0.0)
        
        # Accumulate
        dw_sum += tl.sum(dw, axis=0)
        db_sum += tl.sum(db, axis=0)
    
    # Store final accumulated gradients
    tl.store(FINAL_DW + cols, dw_sum, mask=mask)
    tl.store(FINAL_DB + cols, db_sum, mask=mask)
