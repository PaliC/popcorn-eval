...
def _layer_norm_bwd_dx_fused(DX,  # pointer to the input gradient
    ...
    offs = pid * stride  # base offset for the row

    # Load mean and rstd for the row
    mean = tl.load(Mean + pid)
    rstd = tl.load(Rstd + pid)

    # Initialize accumulators
    dyw_sum = tl.zeros([1], dtype=tl.float32)
    dyw_xmu_sum = tl.zeros([1], dtype=tl.float32)

    # Iterate over the columns in blocks
    for offset in tl.group_range(0, N, BLOCK_SIZE_N):
        idx = offs + offset
        x = tl.load(X + idx)
        dy = tl.load(DY + idx)
        w = tl.load(W + idx)
    ...
    # Reduce sums across the group
    dyw_sum = tl.sum(dyw_sum)
    dyw_xmu_sum = tl.sum(dyw_xmu_sum)

    # Compute means
    mean_dyw = dyw_sum / N
    mean_dyw_xmu = dyw_xmu_sum / N

    # Compute partial sums for DW and DB
    for offset in tl.group_range(0, N, BLOCK_SIZE_N):
        idx = offs + offset
        x = tl.load(X + idx)
        dy = tl.load(DY + idx)
        ...
        # Atomic add for DW
        with tl.atomic_lock(Lock + pid):
            tl.atomic_add(DW + pid, dyx)
            tl.atomic_add(DB + pid, dy)

    # Compute dX and store
    for offset in tl.group_range(0, N, BLOCK_SIZE_N):
        idx = offs + offset
        x = tl.load(X + idx)
        dy = tl.load(DY + idx)
        ...
        dyw = dy * w
        dX_val = (dyw - mean_dyw - (x - mean) * mean_dyw_xmu) * rstd
        tl.store(DX + idx, dX_val)
