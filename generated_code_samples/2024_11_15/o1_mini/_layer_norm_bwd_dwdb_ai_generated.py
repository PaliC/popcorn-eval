...
def _layer_norm_bwd_dwdb(DW,    # pointer to the partial sum of weights gradient
    ...
    grid_m, grid_n = tl.arange(0, BLOCK_SIZE_M), tl.arange(0, BLOCK_SIZE_N)
    rows = pid_m * BLOCK_SIZE_M + grid_m[:, None]
    cols = pid_n * BLOCK_SIZE_N + grid_n[None, :]
    ...
    partial_dw = tl.load(DW + rows[:, None] * N + cols[None, :], mask=mask_m[:, None] & mask_n[None, :], other=0.0)
    partial_db = tl.load(DB + cols, mask=mask_n, other=0.0)
    
    # Reduce partial_dw across the block and accumulate to FINAL_DW
    sum_dw = tl.sum(partial_dw, axis=(0, 1))
    tl.atomic_add(FINAL_DW + cols, sum_dw)
    
    # Reduce partial_db and accumulate to FINAL_DB
    sum_db = tl.sum(partial_db, axis=0)
    tl.atomic_add(FINAL_DB + cols, sum_db)
