...
def _layer_norm_bwd_dx_fused(DX, DY, DW, DB, X, W, Mean, Rstd, Lock, stride, N,
    ...
    # Thread indices for rows and columns
    row_idx = block_start + tl.arange(0, BLOCK_SIZE_N)
    ...
    dy = tl.load(DY + row_idx)  # Output gradient
    x = tl.load(X + row_idx)    # Input
    mean = tl.load(Mean)         # Mean
    rstd = tl.load(Rstd)         # 1/std
    w = tl.load(W)               # Weights

    # Compute the gradients
    dx = dy * rstd               # Gradient w.r.t input
    db = dy                       # Gradient w.r.t biases (accumulated)
    dw = dx * (x - mean)         # Gradient w.r.t weights (accumulated)

    # Accumulate gradients
    with tl.locking(Lock):
        tl.atomic_add(DB, db)
        tl.atomic_add(DW, dw)

    # Store the computed gradient w.r.t input
    tl.store(DX + row_idx, dx)
