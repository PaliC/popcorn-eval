...
def _layer_norm_bwd_dwdb(DW,  # pointer to the partial sum of weights gradient
    ...
    block_end_m = tl.min(block_start_m + BLOCK_SIZE_M, M)
    
    # Accumulate gradients for weights (DW)
    for m in range(block_start_m, block_end_m):
        dw_acc = tl.zeros((N,), dtype=tl.float32)  # accumulator for weights
        db_acc = tl.zeros((N,), dtype=tl.float32)  # accumulator for biases
        for n in range(N):
            dw_acc += DW[m, n]  # accumulate weight gradients
            db_acc += DB[m, n]  # accumulate bias gradients

        # Write accumulated results to final gradients
        FINAL_DW[m] = dw_acc
        FINAL_DB[m] = db_acc
