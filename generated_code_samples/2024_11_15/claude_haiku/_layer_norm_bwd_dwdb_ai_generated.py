...
def _layer_norm_bwd_dwdb(DW,  
    ...
    """
    Compute layer norm backward pass gradients for weights and biases
    """
    # Compute thread/block indices
    pid = tl.program_id(0)
    off_m = pid * BLOCK_SIZE_M + tl.arange(0, BLOCK_SIZE_M)
    off_n = tl.arange(0, BLOCK_SIZE_N)
    
    # Accumulate block-level gradients for weights (DW)
    dw_block_ptr = DW + off_m[:, None] * N + off_n[None, :]
    dw_block = tl.load(dw_block_ptr, mask=(off_m[:, None] < M), other=0.0)
    dw_sum = tl.sum(dw_block, axis=1)
    
    # Accumulate block-level gradients for biases (DB)
    db_block_ptr = DB + off_m[:, None] * N + off_n[None, :]
    db_block = tl.load(db_block_ptr, mask=(off_m[:, None] < M), other=0.0)
    db_sum = tl.sum(db_block, axis=1)
    
    # Store final accumulated gradients 
    mask_m = off_m < M
    if mask_m:
        tl.store(FINAL_DW + off_m, dw_sum)
        tl.store(FINAL_DB + off_m, db_sum)
