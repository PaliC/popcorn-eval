...
def _layer_norm_bwd_dx_fused(DX, DY, DW, DB, X, W, Mean, Rstd, Lock, stride, N,
    ...
    # Get current row and block 
    pid = tl.program_id(axis=0)
    pid_m = pid // tl.cdiv(N, BLOCK_SIZE_N)
    pid_n = pid % tl.cdiv(N, BLOCK_SIZE_N)

    # Compute offsets for input gradients and inputs
    offs_m = pid_m * stride
    offs_n = pid_n * BLOCK_SIZE_N + tl.arange(0, BLOCK_SIZE_N)
    mask = offs_n < N

    # Load output gradient and input
    dy = tl.load(DY + offs_m * stride + offs_n, mask=mask)
    x = tl.load(X + offs_m * stride + offs_n, mask=mask)
    ...
    # Compute local DW and DB 
    local_dw = dy * x_hat
    local_db = dy

    # Reduce DW and DB
    dw = tl.sum(local_dw, axis=0)
    db = tl.sum(local_db, axis=0)

    # Accumulate DW and DB using atomic add with lock
    if tl.get_lane_id() == 0:
        tl.atomic_add(DW + offs_n, dw)
        tl.atomic_add(DB + offs_n, db)

    # Compute dx
    dx_unscaled = dy * w
    dx = dx_unscaled - tl.sum(dx_unscaled, axis=0) / BLOCK_SIZE_N - x_hat * tl.sum(dx_unscaled * x_hat, axis=0)
    dx = dx * rstd

    # Store DX
    tl.store(DX + offs_m * stride + offs_n, dx, mask=mask)
