...
def _layer_norm_bwd_dwdb(DW,  # pointer to the partial sum of weights gradient
    ...
    mask = (off_m < M)[:, None] & (off_n < N)
    
    # Load partial gradient sums from DW and DB
    dw_ptrs = DW + off_m[:, None] * N + off_n
    db_ptrs = DB + off_m[:, None] * N + off_n
    
    # Reduce partial gradient sums across threadblock
    dw_sum = tl.sum(tl.load(dw_ptrs, mask=mask), axis=0)
    db_sum = tl.sum(tl.load(db_ptrs, mask=mask), axis=0)
    
    # Store final reduced gradients to output
    if pid == 0:
        tl.store(FINAL_DW, dw_sum)
        tl.store(FINAL_DB, db_sum)
